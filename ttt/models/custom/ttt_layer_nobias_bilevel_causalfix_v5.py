...
class TTTBase(nn.Module):
    ...
    def setup(self):
        ...
        # Make slow_weight_mix learnable so model can disable it if not helpful
        slow_weight_mix_raw = self.param(
            "slow_weight_mix_logit", 
            nn.initializers.constant(-0.85),  # sigmoid(-0.85) ≈ 0.3
            (), 
            jnp.float32
        )
        # Apply sigmoid to keep it in [0,1] range
        self.slow_weight_mix = nn.sigmoid(slow_weight_mix_raw)
    ...
    def __call__(
        ...
        # Initialize TTT parameter caches if needed
        ...
        def update_embed(XQ, XK, XV, eta, ttt_params=None, ttt_params_slow=None):
            ...
            def parallelize_over_heads(XQ, XK, XV, eta, ttt_params_init, ttt_params_slow_init, ttt_norm_params):
                def compute_mini_batch(carry, inputs):
                    ttt_params_mini_batch_init, ttt_params_slow_current, mini_batch_idx = carry
                    XQ_mini_batch = inputs["XQ"]
                    XK_mini_batch = inputs["XK"]
                    XV_mini_batch = inputs["XV"]
                    eta_mini_batch = inputs["eta"]

                    # Process mini-batch with fast weights (standard TTT)
                    ...
                    # Process mini-batch with slow weights (using slow weights from BEFORE this mini-batch)
                    # This maintains causality because ttt_params_slow_current was computed from previous mini-batches only
                    _, outputs_slow = self.process_mini_batch(
                        ...
                        ttt_params_init,
                        ttt_params_slow_current,  # These contain no information from current mini-batch
                        ttt_norm_params,
                    )
                    ...
                    # Learnable weighted combination - model can set slow_weight_mix to 0 to disable slow path
                    ...
                    # Update slow weights for FUTURE mini-batches (doesn't affect current output)
                    # This maintains causality: slow weights for mini-batch t+1 are based on fast weights from mini-batch t
                    should_update_slow = (mini_batch_idx + 1) % self.slow_update_freq == 0
                    ttt_params_slow_updated = jax.lax.cond(
                        should_update_slow,
                        lambda _: self.update_slow_weights(ttt_params_slow_current, ttt_params_last_in_mini_batch),
                ...
                # Initialize with slow weights that have not seen any tokens from current sequence
                initial_mini_batch_idx = jnp.array(0)
                initial_carry = (ttt_params_init, ttt_params_slow_init, initial_mini_batch_idx)
                ...
                ttt_params_final, ttt_params_slow_final, _ = final_carry
    ...
    def update_slow_weights(self, ttt_params_slow_current, ttt_params_fast_current):
        """Update slow weights with exponential moving average.
        
        Args:
            ttt_params_slow_current: Current slow weights (from previous mini-batches)
            ttt_params_fast_current: Fast weights after processing current mini-batch
        
        Returns:
            Updated slow weights that will be used in future mini-batches
        """
        # Exponential moving average update - conservative to maintain stability
        alpha = 0.05  # Small mixing coefficient for gradual adaptation
