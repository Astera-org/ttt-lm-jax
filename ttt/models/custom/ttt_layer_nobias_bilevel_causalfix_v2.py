        ...
        def update_embed(XQ, XK, XV, eta, ttt_params=None, ttt_params_slow=None):
            ...
            def parallelize_over_heads(XQ, XK, XV, eta, ttt_params_init, ttt_params_slow_init, ttt_norm_params):
                ...
                def compute_mini_batch(carry, inputs):
                    """
                    CAUSALITY-FIXED IMPLEMENTATION
                    
                    Key insight: Slow weights must start each mini-batch clean (no future info)
                    but can adapt during mini-batch processing (preserving causality within mini-batch).
                    The dual form's jnp.tril() ensures causality within mini-batch processing.
                    """
                    ...
                    # CAUSALITY FIX: Process with slow weights that haven't seen current mini-batch
                    # but allow them to adapt during processing (maintaining causality within mini-batch)
                    ttt_params_slow_after_current, outputs_slow = self.process_mini_batch(
                        XQ_mini_batch,
                        XK_mini_batch,
                        XV_mini_batch,
                        eta_mini_batch * 0.1,  # Slower learning rate
                        ttt_params_init,
                        ttt_params_slow_mini_batch_init,  # Clean starting point - no future info
                        ttt_norm_params,
                    )
                    
                    # Update slow weight baseline for future mini-batches only
