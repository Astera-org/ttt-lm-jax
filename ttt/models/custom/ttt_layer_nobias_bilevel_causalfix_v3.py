...
class TTTBase(nn.Module):
    ...
    def setup(self):
        ...
        # Mixing coefficient for combining fast and slow weights
        self.slow_weight_mix = getattr(self.config, 'slow_weight_mix', 0.3)
        
        # Exponential moving average coefficient for slow weight updates
        ...
        self.ttt_params = ()
        self.ttt_params_slow = ()  # Second set of TTT weights
        ...
        def update_embed(XQ, XK, XV, eta, ttt_params=None, ttt_params_slow=None):
            ...
            def parallelize_over_heads(XQ, XK, XV, eta, ttt_params_init, ttt_params_slow_init, ttt_norm_params):
                def compute_mini_batch(carry, inputs):
                    # Unpack carry - now includes previous fast weights for slow weight updates
                    ttt_params_mini_batch_init, ttt_params_slow_mini_batch_init, mini_batch_idx, prev_fast_weights = carry
                    ...
                    # CAUSALITY FIX: Process mini-batch with fast weights
                    ttt_params_last_in_mini_batch, outputs_fast = self.process_mini_batch(
                        ...
                        eta_mini_batch,
                        ttt_params_init,
                        ttt_params_mini_batch_init,  # Fast weights from start of mini-batch
                        ttt_norm_params,
                    )
                    
                    # CAUSALITY FIX: Process mini-batch with slow weights (using weights from start of mini-batch)
                    # These slow weights only contain information from previous mini-batches
                    _, outputs_slow = self.process_mini_batch(
                        ...
                        ttt_params_init,
                        ttt_params_slow_mini_batch_init,  # Slow weights from start of mini-batch (causal)
                        ttt_norm_params,
                    )
                    ...
                    # CAUSALITY FIX: Update slow weights using previous mini-batch's fast weights
                    # This ensures slow weights never see future information
                    should_update_slow = (mini_batch_idx + 1) % self.slow_update_freq == 0
                    ttt_params_slow_updated = jax.lax.cond(
                        should_update_slow,
                        # Use previous fast weights if available, otherwise keep current slow weights
                        lambda _: self.update_slow_weights(ttt_params_slow_mini_batch_init, prev_fast_weights) if prev_fast_weights is not None else ttt_params_slow_mini_batch_init,
                        lambda _: ttt_params_slow_mini_batch_init,
                        None
                    )
                    
                    # Store current fast weights as "previous" for next iteration
                    new_prev_fast_weights = ttt_params_last_in_mini_batch
                    
                    new_carry = (ttt_params_last_in_mini_batch, ttt_params_slow_updated, mini_batch_idx + 1, new_prev_fast_weights)
                    return new_carry, outputs_combined

                inputs = {"XQ": XQ, "XK": XK, "XV": XV, "eta": eta}
                
                # Initialize with None for prev_fast_weights since we don't have previous mini-batch initially
                initial_mini_batch_idx = jnp.array(0)
                initial_carry = (ttt_params_init, ttt_params_slow_init, initial_mini_batch_idx, None)
                ...
                ttt_params_final, ttt_params_slow_final, _, _ = final_carry
                Z, ttt_loss_mse_init, ttt_loss_mse_step_0, ttt_loss_mse_step_1 = outputs

                return (Z.reshape(-1, self.head_dim), ttt_loss_mse_init, ttt_loss_mse_step_0, ttt_loss_mse_step_1, ttt_params_final, ttt_params_slow_final)
    ...
    def update_slow_weights(self, ttt_params_slow_current, ttt_params_fast_prev):
        """Update slow weights based on previous mini-batch's fast weights with exponential moving average."""
        # Exponential moving average update using previous fast weights
        alpha = self.slow_weight_alpha
        
        def update_param(slow_param, fast_param):
            return (1 - alpha) * slow_param + alpha * fast_param
        
        return tree_map(update_param, ttt_params_slow_current, ttt_params_fast_prev)
...
class TTTLinearBase(TTTBase):
    ...
    def process_mini_batch(
            ...
            # Original adaptive behavior - dual form preserves causality through lower triangular attention
            X1_bar = XQ_mini_batch
            Attn1 = jnp.tril(X1_bar @ X1.transpose(1, 0))  # Lower triangular preserves causality
...
class TTTMLPBase(TTTBase):
    ...
    def process_mini_batch(
        ...
        Attn2 = jnp.tril(X2_bar @ X2.transpose(1, 0))  # Lower triangular preserves causality
