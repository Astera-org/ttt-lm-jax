...
class TTTBase(nn.Module):
    ...
    def process_mini_batch(
        ...
        """
        FIXED: Process a mini-batch with proper temporal ordering to avoid data leakage.
        
        Key fix: Use slow weights BEFORE updating them, not after.
        ...
        Args:
            ...
            ttt_params_slow_mini_batch_init: Current slow TTT parameters (from PREVIOUS updates)
            ttt_norm_params: Layer norm parameters
            mini_batch_idx: Current mini-batch index
            slow_update_freq: Frequency of slow weight updates
        
        Returns:
            tuple: (new_fast_params, new_slow_params, combined_outputs)
        """
        
        # STEP 1: Process with fast weights (standard TTT)
        ...
        # STEP 2: Process with slow weights (using CURRENT slow weights, before updating)
        # This ensures no data leakage - slow weights haven't seen this mini-batch yet
        _, slow_outputs = self._process_mini_batch_with_params(
            ...
            ttt_params_init, ttt_params_slow_mini_batch_init, ttt_norm_params
        )
        
        # STEP 3: Combine outputs from fast and slow weights
    ...
    #   output_mini_batch_combined = frobenius_normalize(output_mini_batch_combined)
        
        # STEP 4: Update slow weights AFTER processing (for future mini-batches)
        ...
        def update_slow_weights_branch():
            # Update slow weights based on the updated fast weights
            # These will be used for FUTURE mini-batches, not the current one
        ...
        def update_embed(XQ, XK, XV, eta, ttt_params=None, ttt_params_slow=None):
            ...
            def parallelize_over_heads(XQ, XK, XV, eta, ttt_params_init, ttt_params_slow_init, ttt_norm_params):
                def compute_mini_batch(carry, inputs):
                    ttt_params_mini_batch_init, ttt_params_slow_mini_batch_init, mini_batch_idx = carry
