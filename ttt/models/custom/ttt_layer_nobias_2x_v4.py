...
class TTTLinearBase(TTTBase):
    def setup(self):
        ...
        # 2x larger TTT parameters: W1 expands to 2*head_dim, W2 contracts back to head_dim
        self.W1 = self.param(
            "ttt_dense_0",
            nn.initializers.normal(self.config.initializer_range),
            (self.num_heads, self.head_dim, 2 * self.head_dim),  # Expanded to 2x
            self.param_dtype,
        )
        ...
        self.W2 = self.param(
            "ttt_dense_1", 
            nn.initializers.normal(self.config.initializer_range),
            (self.num_heads, 2 * self.head_dim, self.head_dim),  # Contract back to head_dim
            self.param_dtype,
        )
    ...
    def process_mini_batch(
            ...
            X1 = XK_mini_batch
            Z1 = X1 @ W1_init  # Shape: [..., 2*head_dim]
            X2 = nn.gelu(Z1)   # Apply activation
            Z2 = X2 @ W2_init  # Shape: [..., head_dim]
            ...
            # Backprop through W2 and activation
            ...
            # Original adaptive behavior for queries
            ...
            # Calculate ttt loss using the updated weights by the current mini-batch
            if self.config.output_ttt_stats:
