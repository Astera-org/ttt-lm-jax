...
class TTTBase(nn.Module):
    ...
    def setup(self):
        ...
        # Slow weight update frequency (every N mini-batches)
        self.slow_update_freq = getattr(self.config, 'slow_update_freq', 16)
        
        # Learnable gating for slow weight influence - allows model to disable mechanism if needed
        self.slow_weight_gate = self.param(
            "slow_weight_gate",
            lambda key, shape, dtype: jnp.array(0.3, dtype=dtype),  # Initialize to small positive value
            (),
            self.param_dtype,
        )
        
        # EMA coefficient for slow weight updates
        self.slow_weight_alpha = getattr(self.config, 'slow_weight_alpha', 0.1)
        ...
        self.ttt_params = ()
        self.ttt_params_slow = ()  # Slow weights - updated less frequently

        # Initialize caches for streaming inference
    ...
    def _apply_causal_conv_with_cache(self, x, conv_layer, cache_key, batch_size):
        ...
        Maintains causality by only using past context.
        """
    ...
    def __call__(
        ...
        _ttt_loss_mse_init, _ttt_loss_mse_step_0, _ttt_loss_mse_step_1, ttt_params_final, ttt_params_slow_final = ttt_stats

        # Update caches with final parameters
    ...
    def ttt(self, XQ, XK, XV, eta):
        ...
        def update_embed(XQ, XK, XV, eta, ttt_params=None, ttt_params_slow=None):
            @partial(vmap, axis_name="head")
            def parallelize_over_heads(XQ, XK, XV, eta, ttt_params_init, ttt_params_slow_init, ttt_norm_params):
                def compute_mini_batch(carry, inputs):
                    # CAUSALITY FIX: Proper separation of fast and slow weights
                    ttt_params_fast_current, ttt_params_slow_current, mini_batch_idx = carry
                    XQ_mini_batch = inputs["XQ"]
                    XK_mini_batch = inputs["XK"]
                    XV_mini_batch = inputs["XV"]
                    eta_mini_batch = inputs["eta"]

                    # Process mini-batch with fast weights (preserves causality via dual form)
                    # Fast weights are initialized from slow weights that only contain past information
                    ttt_params_fast_after_processing, outputs = self.process_mini_batch(
                        XQ_mini_batch,
                        XK_mini_batch,
                        XV_mini_batch,
                        eta_mini_batch,
                        ttt_params_init,  # Original initialization (stays constant)
                        ttt_params_fast_current,  # Current fast weights (warm-started from slow weights)
                        ttt_norm_params,
                    )
                    
                    output_mini_batch, ttt_loss_mse_init, ttt_loss_mse_step_0, ttt_loss_mse_step_1 = outputs
                    
                    # Update slow weights at END of mini-batch using final fast weights
                    # This information will only be used for FUTURE mini-batches
                    should_update_slow = (mini_batch_idx + 1) % self.slow_update_freq == 0
                    ttt_params_slow_updated = jax.lax.cond(
                        should_update_slow,
                        lambda _: self.update_slow_weights(ttt_params_slow_current, ttt_params_fast_after_processing),
                        lambda _: ttt_params_slow_current,
                        None
                    )
                    
                    # For the NEXT mini-batch, initialize fast weights from slow weights
                    # Apply learnable gating to allow model to control influence of slow weights
                    slow_gate = nn.sigmoid(self.slow_weight_gate)
                    
                    def mix_weights(slow_param, fast_param):
                        return slow_gate * slow_param + (1.0 - slow_gate) * fast_param
                    
                    # When slow weights are updated, use them as initialization for next fast weights
                    # Otherwise, continue with current fast weights
                    next_fast_init = jax.lax.cond(
                        should_update_slow,
                        lambda _: tree_map(mix_weights, ttt_params_slow_updated, ttt_params_fast_after_processing),
                        lambda _: ttt_params_fast_after_processing,
                        None
                    )
                    
                    # Output is purely from causal fast weight processing
                    outputs_final = (output_mini_batch, ttt_loss_mse_init, ttt_loss_mse_step_0, ttt_loss_mse_step_1)
                    
                    new_carry = (next_fast_init, ttt_params_slow_updated, mini_batch_idx + 1)
                    return new_carry, outputs_final

                inputs = {"XQ": XQ, "XK": XK, "XV": XV, "eta": eta}
                
                # Initialize: fast weights start from slow weights (maintains causality)
                initial_mini_batch_idx = jnp.array(0)
                initial_carry = (ttt_params_slow_init, ttt_params_slow_init, initial_mini_batch_idx)
                ...
                ttt_params_fast_final, ttt_params_slow_final, _ = final_carry
                Z, ttt_loss_mse_init, ttt_loss_mse_step_0, ttt_loss_mse_step_1 = outputs

                return (Z.reshape(-1, self.head_dim), ttt_loss_mse_init, ttt_loss_mse_step_0, ttt_loss_mse_step_1, ttt_params_fast_final, ttt_params_slow_final)
    ...
    def update_slow_weights(self, ttt_params_slow_current, ttt_params_fast_current):
        """Update slow weights using exponential moving average of fast weights."""
        alpha = self.slow_weight_alpha
...
class TTTLinearBase(TTTBase):
    ...
    def process_mini_batch(
            ...
            # Dual form computation (preserves causality via lower triangular mask)
...
class TTTMLPBase(TTTBase):
    ...
    def process_mini_batch(
        ...
        # Dual form computation (preserves causality)
        X1_bar = XQ_mini_batch
        Attn1 = jnp.tril(X1_bar @ X1.transpose(1, 0))
        Z1_bar = X1_bar @ W1_init - (square_eta_mini_batch * Attn1) @ grad_l_wrt_Z1
