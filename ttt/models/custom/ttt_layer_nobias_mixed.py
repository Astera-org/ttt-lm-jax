...
class TTTBase(nn.Module):
    ...
    def setup(self):
        ...
        # Force TTT computation dtype to fp32 for numerical stability
        self.ttt_compute_dtype = jnp.float32
    ...
    def ttt(self, XQ, XK, XV, eta):
        """TTT computation with forced fp32 for numerical stability"""
        ...
        def update_embed(XQ, XK, XV, eta, ttt_params=None):
            ...
            def parallelize_over_heads(XQ, XK, XV, eta, ttt_params_init, ttt_norm_params):
                def compute_mini_batch(ttt_params_mini_batch_init, inputs):
                    # Cast everything to fp32 for TTT computation
                    XQ_mb = inputs["XQ"].astype(self.ttt_compute_dtype)
                    XK_mb = inputs["XK"].astype(self.ttt_compute_dtype)
                    XV_mb = inputs["XV"].astype(self.ttt_compute_dtype)
                    eta_mb = inputs["eta"].astype(self.ttt_compute_dtype)
                    
                    # Cast parameters to fp32
                    ttt_params_fp32 = jax.tree_map(
                        lambda x: x.astype(self.ttt_compute_dtype), 
                        ttt_params_mini_batch_init
                    )
                    ttt_params_init_fp32 = jax.tree_map(
                        lambda x: x.astype(self.ttt_compute_dtype), 
                        ttt_params_init
                    )
                    ttt_norm_params_fp32 = jax.tree_map(
                        lambda x: x.astype(self.ttt_compute_dtype), 
                        ttt_norm_params
                    )

                    ttt_params_last, outputs = self.process_mini_batch(
                        XQ_mb, XK_mb, XV_mb, eta_mb,
                        ttt_params_init_fp32,
                        ttt_params_fp32,
                        ttt_norm_params_fp32,
                    )
                    
                    # Cast output back to original dtype
                    output_mb, loss_init, loss_0, loss_1 = outputs
                    output_mb = output_mb.astype(self.dtype)
                    
                    # Keep parameters in param_dtype for storage efficiency
                    ttt_params_last = jax.tree_map(
                        lambda x: x.astype(self.param_dtype), 
                        ttt_params_last
                    )
                    
                    return ttt_params_last, (output_mb, loss_init, loss_0, loss_1)
        ...
        outputs = update_embed(
            XQ, XK, XV, eta, 
            ttt_params=self.ttt_cache.value if hasattr(self, 'ttt_cache') else None
        )
...
class TTTLinearBase(TTTBase):
    ...
    def process_mini_batch(
            ...
            # All inputs are already in fp32 from the caller
